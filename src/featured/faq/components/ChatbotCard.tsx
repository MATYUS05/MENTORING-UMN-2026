// src/featured/faq/components/ChatbotCard.tsx
  ...
  if (!isOpen) {
    return (
      <button
        ...
        className="flex w-full items-center justify-center gap-3 rounded-2xl border-2 border-neutral-stone/30 bg-neutral-surface p-8 text-center transition-all duration-200 hover:-translate-y-0.5 hover:border-secondary-deep/50 hover:shadow-md dark:border-neutral-stone/20 dark:bg-neutral-charcoal dark:hover:border-secondary-sky/40"
      >
        <img src={neutral} alt="Chatbot" className="h-8 w-8" />
        <span className="font-body text-base font-semibold text-neutral-charcoal dark:text-neutral-cream">Tanya Chatbot</span>
      </button>
    );
  }

  return (
    <div className="flex h-112 flex-col overflow-hidden rounded-2xl border-2 border-neutral-stone/30 bg-neutral-surface dark:border-neutral-stone/20 dark:bg-neutral-charcoal">
      <div className="flex items-center justify-between bg-secondary-deep px-4 py-3 text-white dark:bg-secondary-sky dark:text-neutral-charcoal-deep">
        <div className="flex items-center gap-2">
          <img src={emotionAssets[emotion]} alt={emotion} className="h-6 w-6" />
          <span className="font-body font-semibold">Tanya Chatbot</span>
        </div>
        <button onClick={() => setIsOpen(false)} className="text-white/80 transition-colors duration-200 hover:text-white dark:text-neutral-charcoal-deep/70 dark:hover:text-neutral-charcoal-deep">
          ✕
        </button>
      </div>
      <div className="flex-1 space-y-3 overflow-y-auto px-4 py-4">
        {messages.length === 0 && (
          <p className="font-body text-sm text-neutral-stone">Ketik pertanyaan kamu di bawah untuk mulai.</p>
        )}
        {messages.map((msg) => (
          <div key={msg.id} className={`flex items-end gap-2 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
            {msg.role === 'bot' && (
              <img src={emotionAssets[msg.emotion ?? 'neutral']} alt={msg.emotion} className="h-6 w-6 shrink-0" />
            )}
            <span
